...
- `affiliate_fee`:The total affiliate fee in basis points (bps), more details [here](/developers/affiliate-fee).
...
You can also authenticate via Sign-In With Ethereum (SIWE). This flow returns a JWT for use in subsequent requests. Refer to [Authentication](/developers/sdk/authentication) for more details.
...
For **EVM** chains, initiation can be done directly via [smart contract](/developers/contracts/atomic-swap-evm.md) or Garden's API.
