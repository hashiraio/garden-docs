...
Affiliates can claim their earnings by first checking available amounts through the API, then submitting an on-chain transaction to the distributor contract.
...
<RequestExample>
```bash cURL
curl -X 'GET' 'https://api.garden.finance/v2/affiliates/earnings' \
...
async function claimAffiliateEarnings(claimData) {
  ...
  const abi = parseAbi([
    'function claim(tuple(address toAddress, address tokenAddress, uint256 totalRewards) _claim, bytes _signature) external'
  ]);

  const claimParams = {
    toAddress: claimData.affiliate,
    tokenAddress: claimData.token_address,
    totalRewards: BigInt(claimData.claim_amount)
  };
