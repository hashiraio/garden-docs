...
<Info>
  Fees are expressed in basis points (bps), where 1 bps = 0.01%. For example, a 30 bps fee equals
  0.3% of the source asset value.
</Info>
...
<Note>
  The amount of each asset the affiliate will receive is calculated based on prices at the time of
  the quote and is also stored in the order data. All affiliate fees collected during the week are
  distributed to the specified addresses at the end of the week.
</Note>
...
<Note>
  Check out the full SDK implementation for [React](/developers/sdk/react/quickstart) or
  [NodeJS](/developers/sdk/nodejs/quickstart) here.
</Note>
