---
title: "Route Policy System"
description: "Learn how to use the route policy system to validate asset trading pairs locally and reduce API dependencies"
---
...
<Tip>
This approach significantly reduces API calls and enables real-time route validation in your application UI.
</Tip>
...
When validating a route, the system checks in this order:
...
<RequestExample>
```bash cURL
curl -X GET 'https://{environment}.garden.finance/v2/policy' \
  -H 'garden-app-id: {api-key}' \
...
Response status - either "Ok" or "Error"
</ResponseField>

<ResponseField name="result" type="object" required>
The route policy configuration object

<Expandable title="Policy Configuration">
<ResponseField name="default" type="string" required>
Default policy for unconfigured routes. Either "open" (allow) or "closed" (deny).
</ResponseField>

<ResponseField name="isolation_groups" type="array">
Array of isolation group rules. Assets in isolation groups can only trade with other assets in the same group.

Format: `"fromAsset <-> toAsset"` or `"chain:asset <-> chain:asset"`
</ResponseField>

<ResponseField name="blacklist_pairs" type="array">
Array of forbidden trading pairs. Supports wildcard patterns.

Format: `"fromAsset -> toAsset"` with wildcards like `"bitcoin:BTC -> *:WBTC"`
</ResponseField>

<ResponseField name="whitelist_overrides" type="array">
Array of explicitly allowed routes that bypass other restrictions.

Format: `"fromAsset -> toAsset"` with optional wildcard support
</ResponseField>
</Expandable>
</ResponseField>
...
<Warning>
Always handle policy loading errors gracefully. You may want to implement retry logic or fallback to server-side validation.
</Warning>
</Step>

<Step title="Validate Routes">
...
Implement policy refresh logic for your application:

```typescript
// Refresh policy periodically or on user action
async function refreshPolicy() {
  try {
    await validator.loadPolicy();
    console.log('Policy refreshed');
  } catch (error) {
...
setInterval(refreshPolicy, 5 * 60 * 1000);
```
</Step>
</Steps>
...
<Tip>
**Cache Policy Data**: Store the policy configuration locally and refresh it periodically rather than fetching it on every route validation.
</Tip>

<Warning>
**Handle Policy Updates**: Implement proper error handling and refresh the policy when a get quote or create order fails, with an unsupported trading pair.
</Warning>

<Info>
**Performance**: The local validation is significantly faster than API calls, making it ideal for real-time UI updates and user experience optimization.
</Info>