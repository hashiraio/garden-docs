...
<Steps>
  <Step title="Create Transaction">
    ...
    - The script address is derived from the HTLC parameters (pubkeys, secret hash, timelock and destination data)
  </Step>
...
<Note>
Unlike EVM contracts where initiate details are known when the initiate happens on chain, in bitcoin only the spending branch details are made public. If redeem happens, only the redeemer public key and secret are made public, where as in the refund case only the initiator is made public.
</Note>
...
<Steps>
  ...
  <Step title="Script Address Generation">
    Generate the script address using Taproot's address format for better privacy, efficiency and cost optimisation.
  </Step>
