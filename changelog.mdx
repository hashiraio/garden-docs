...
<Update label="July 30, 2025" description="v2.4.0" tags={["SDK"]}>
   - Add `handleSecretManagement` flag for optional secret management. When set to `true`, the relayer manages order redemption, eliminating the need for the user to keep the browser open after initiating a swap. When set to `false`, the user must handle secrets and keep the browser open until redemption is complete.
</Update>
