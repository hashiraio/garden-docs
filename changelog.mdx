...
<Update label="July 30, 2025" description="v2.4.0" tags={["SDK"]}>
  - Add optional secret management to SDK. This is configured via the `handleSecretManagement` flag: when set to `true`, the relayer manages order redemption, eliminating the need for the user to keep the browser open after initiating a swap; when set to `false`, the user must handle secrets and keep the browser open until redemption is complete.
</Update>
...
<Update label="July 30, 2025" description="v2.3.2" tags={["SDK"]}>
  - Add Instant refund SACP call for expired orders for automatic refunds.
</Update>
...
<Update label="July 16, 2025" description="v2.3.0" tags={["SDK"]}>
  - Move `@catalogfi/wallets` dependency to Garden SDK.
</Update>
