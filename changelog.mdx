...
<Update label="January 12, 2026" description="v2.0.15" tags={["API"]}>
  ...
  - Update PATCH `/orders/:id` to no longer accept `action=refund`, and `action=instant-refund` requires `signatures` array instead of `signature` string
</Update>

<Update label="January 1, 2026" description="v2.0.14" tags={["API"]}>
  ### Endpoint updates
  - Update GET `/orders` to include `solver_id` field
  - Update GET `/orders` to accept `solver_id` as a query parameter
</Update>
